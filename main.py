...
def create_video(values):
    ...
    if text_file != "" and audio_clip != None:
        ...
        sent_dur = (int(values.get(DUR_PER_WORD)) * int(values.get(NUM_WORDS))) / 1000
        text_disp_dur = int(values.get(TEXT_DISP_DUR))
        ...
        while j < len(sentences):
            if start_sec <= text_disp_dur - sent_dur:
                time_in_sec = (start_min * 60) + start_sec
                if time_in_sec <= total_video_dur:
                    sent = " ".join(sentences[j])
                    sent = TextClip(txt=sent, color=font_color, fontsize=font_size, font=font)
                    sent = sent.set_position(text_pos)
                    sent = sent.set_start(time_in_sec)
        ...
        output_video.write_videofile(output_path, fps=24, audio_codec="aac")
...
layout = [ [sg.Text(text="Text file:"), sg.FileBrowse(key=TEXT_FILE_PATH, tooltip="Text file")],
           ...
           # [sg.Text(text="Duration per word (ms)"), sg.InputText(size=(10, 0), key=DUR_PER_WORD, default_text="1000")],
           # [sg.Text(text="Number of words:"), sg.InputText(size=(10, 10), key=NUM_WORDS, default_text="5")],
           # [sg.Text(text="Text display duration per minute (s):"), sg.Slider(key=TEXT_DISP_DUR, range=(1, 60), default_value=60, orientation="horizontal")],
           [sg.Button("Submit")]
            ]
